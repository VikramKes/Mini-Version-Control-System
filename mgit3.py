...
#unix methods start
mygit_floger_perfect_path=''
my_project_perfect_path=''
def add_the_difference(accu_file,diff_file):#arguments cointain the complete path
    global mygit_floger_perfect_path
...
def delete_instant(file_name):
    ...
    if match_obj:
        os.remove(mygit_floger_perfect_path+file_name)

#helper method
def log_entry(comment):
    global mygit_floger_perfect_path
    log_file=open(mygit_floger_perfect_path+"log",'a')
    master_file=open(mygit_floger_perfect_path+'master_file.txt','r')
...
def log_read():
    global mygit_floger_perfect_path
    f=open(mygit_floger_perfect_path+'master_file.txt')
    a=int(f.read())
    f.close()
    if a==0:
        print('no commit done yet')
        exit()
    lfd=open(mygit_floger_perfect_path+'log','r')
    for i in lfd:
        print(i)
    lfd.close()
...
def current_files():
    global mygit_floger_perfect_path
    global my_project_perfect_path
    #check all the file exept from mygit
    res=[]
    #forbiden_path=os.getcwd()+'/.mygit'
    forbiden_path=mygit_floger_perfect_path[:-1]#if this does not work then remove last '/'
    print('the fobidden path is ',forbiden_path)
    for root,dirs,files in os.walk(my_project_perfect_path):#this directory needs to be changed.
        temp_root=root
        #print('root is equal to ',root)
        #print('forbiden path is equal to ',forbiden_path)
...
def extract_commit(commit_no):
    ...
    #current_commit_num=master_file.read()
    current_commit_file_name=mygit_floger_perfect_path+'commit_'+str(commit_no)
...
def extract_previous_commit_info():
    global mygit_floger_perfect_path
     #this will return list of file name and there hash and ther diff file n.o
    #print('the path is '+mygit_floger_perfect_path+'master_file.txt')
    master_file=open(mygit_floger_perfect_path+'master_file.txt','r')
    current_commit_num=master_file.read()
    current_commit_file_name=mygit_floger_perfect_path+'commit_'+current_commit_num
    current_commit_file=open(current_commit_file_name,'r')
    num_of_file=current_commit_file.readline()#reading the number of file.
    num_of_file=num_of_file[:-1]
    my_map={}
...
def extract_previous_add_info():
    global mygit_floger_perfect_path
    only_add_file=open(mygit_floger_perfect_path+'addlist_unique.txt')
    num_co_file=open(mygit_floger_perfect_path+'myfile_counter.txt','r')
...
def file_changed():  #bugs to be fixed
    ...
    if os.path.exists(mygit_floger_perfect_path+'addlist_unique.txt'):
...
def recover_file_from_commit(file_path,created_file_name):#this will cointain the complete file path
    ...
    if file_path not in temp_commit_map.keys():
        ...
        while len(mystack)!=0:
            ...
            add_the_difference(created_file_name,mygit_floger_perfect_path+'diff'+str(target_diff_index))
...
def commit_routine(mycomment):
    ...
    if os.path.exists(mygit_floger_perfect_path+'addlist_unique.txt'):
        ...
        for e_f in add_file_map.keys():
            ...
            diff_count_filed=open(mygit_floger_perfect_path+'diff_counter','r')
            new_diff_index=int(diff_count_filed.read())
            diff_count_filed.close()
            diff_count_filed=open(mygit_floger_perfect_path+'diff_counter','w')
            diff_count_filed.write(str(new_diff_index+1))
            diff_count_filed.close()
            #rename the file
            os.rename(mygit_floger_perfect_path+'tempdiff'+str(diff_index),mygit_floger_perfect_path+'diff'+str(new_diff_index))
        ...
        master_filed=open(mygit_floger_perfect_path+'master_file.txt','r')
        commit_no=int(master_filed.read())
        commit_no=commit_no+1
        master_filed.close()
        commit_filed=open(mygit_floger_perfect_path+'commit_'+str(commit_no),'w')
        ...
        os.remove(mygit_floger_perfect_path+'addlist_unique.txt')
        os.remove(mygit_floger_perfect_path+'myfile_counter.txt')
        os.remove(mygit_floger_perfect_path+'add_diff.txt')
        #increase the counter of master file.
        master_filed=open(mygit_floger_perfect_path+'master_file.txt','w')
...
def text_check(x):
    mime=mimetypes.guess_type(x)
    print('path hero is ',x)
    print(mime)
    if mime[0] is None:
        return False
    if(mime[0][0:4]=="text"):
...
def add(x):
    ...
    #x=os.path.abspath(x)
    #checking whether the file is text file or not.
    if text_check(x):
    ...
    if x in changed_fil:
        if os.path.exists(mygit_floger_perfect_path+'addlist_unique.txt'):
            ...
            add_file_dis=open(mygit_floger_perfect_path+'addlist_unique.txt','a')
            ...
            if x in add_map.keys():
                ...
                extra_file=open(mygit_floger_perfect_path+'addlist_unique.txt','w')
                ...
                temp_counter_file=open(mygit_floger_perfect_path+'myfile_counter.txt','r')
                t=int(temp_counter_file.read())
                t=t-1
                temp_counter_file.close()
                temp_counter_file=open(mygit_floger_perfect_path+'myfile_counter.txt','w')
            ...
            add_file_dis=open(mygit_floger_perfect_path+'addlist_unique.txt','w')
            mytempt=open(mygit_floger_perfect_path+'myfile_counter.txt','w')
            mytempt.write('0')
            mytempt.close()
            mytempt=open(mygit_floger_perfect_path+'add_diff.txt','w')
            mytempt.write('1')
            mytempt.close()
        ...
        current_diff_counter=open(mygit_floger_perfect_path+'add_diff.txt','r')
        diff_val=int(current_diff_counter.read())
        current_diff_counter.close()
        current_diff_counter=open(mygit_floger_perfect_path+'add_diff.txt','w')
        ...
        if x in my_map.keys():
            ...
            recover_file_from_commit(x,mygit_floger_perfect_path+'marvel_is_better_than_dc.txt')
            #now we need to create the temp diff file
            temp_diff_file_name=mygit_floger_perfect_path+"tempdiff"+str(diff_val)
            create_diff(mygit_floger_perfect_path+'marvel_is_better_than_dc.txt',x,temp_diff_file_name)
            add_file_dis.writelines(str(diff_val)+'\n')
            os.popen('rm '+mygit_floger_perfect_path+' marvel_is_better_than_dc.txt')
            ...
            #do he diffing here
            create_diff(mygit_floger_perfect_path+'glob_empty',x,temp_diff_file_name)
            add_file_dis.writelines(str(diff_val)+'\n')
...
def pull():
    ...
    os.popen('rm -r '+mygit_floger_perfect_path)
    s = socket.socket()             
    host = socket.gethostname()     
    port = 60000                   
    s.connect((host, port))
    s.send("PULL".encode())
    #s.send(bytes("PULL","utf-8"))
    empty_zip_data = "PK\x05\x06\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
    with open(my_project_perfect_path+'/mygit.zip', 'wb') as wf:
        wf.write(bytes(empty_zip_data,"utf-8"))
        while True:
        ...
        wf.close()
        time.sleep(2)
	#os.popen('unzip /home/hp/Documents/DirTest/result/mygit.zip -d /home/hp/Documents/DirTest/result/.mygit')
    with ZipFile(my_project_perfect_path+'/mygit.zip', 'r') as zipObj:
        zipObj.extractall(my_project_perfect_path)
        zipObj.close()
...
def push():
    ...
    s.connect((host, port))
    s.send("PUSH".encode())
    path=my_project_perfect_path
    x=os.popen(' zip -r '+ 'mygit '+'.mygit')
    path+="/mygit.zip"
    while not os.path.exists(path):
        print("inside")
    time.sleep(2)
		#path1="/home/hp/Documents/Dir Test/result/mygit.zip"
			#filename='mytext.txt'
    f = open(path,'rb')
			#time.sleep(10)
    statinfo = os.stat('mygit.zip')
    lsize=statinfo.st_size
			#l = f.read(1024)
    i=0
    while i<lsize:
        rrr=f.read(1)
...
#lets see it work.
if a[1]!='init':
    #print('the method is not init')
    my_cwd=os.getcwd()
    mylist=my_cwd.split('/')
    while len(mylist)!=0:
        temp_path=''#this may now give an error
        
        for i in mylist:
            temp_path=temp_path+i+'/'
            
        #print('curret path is ',temp_path)
        l=next(os.walk(temp_path))[1]
        print(l)
        if '.mygit' in l:
            mygit_floger_perfect_path=temp_path+'.mygit/'
            #print('value shored int the variable is '+mygit_floger_perfect_path)
            my_project_perfect_path=temp_path
            break
        mylist.pop()
    if mygit_floger_perfect_path=='':
        print('not able to find .mygit folder please do init before procedding...')
        exit()
...
if a[1]=='init':
    if len(a)!=2:
        ...
        if(os.path.exists('.mygit')):
            print('.mygit file already exity inisializing git multiple time is not allowed')
            exit()
        #print('correct implementation starts from here')
        #we need to create a master file
        os.mkdir('.mygit')
        mygit_floger_perfect_path=os.path.abspath('.mygit')
        mygit_floger_perfect_path=mygit_floger_perfect_path+'/'
        my_project_perfect_path=os.getcwd()
        #print('the path of the git folder is '+mygit_floger_perfect_path)
        if os.path.exists('.mygit/glob_empty'):
            pass
        else:
            my_emp=open('.mygit/glob_empty','wb')
            my_emp.close()
